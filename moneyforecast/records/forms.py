...
class RecordForm(forms.ModelForm):
    ...
    type_category = forms.IntegerField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if type_category_pk:
            self.fields['category'].queryset = Category.objects.filter(
                type_category=type_category_pk, user=self.user)
        else:
            try:
                self.fields['category'].queryset = Category.objects.filter(
                    type_category=self.instance.category.type_category,
                    user=self.user)
            except:
                self.fields['category'].queryset = Category.objects.filter(
                    user=self.user)
    ...
    def clean_category(self):
        ...
        if data['new_category']:
            try:
                cat_instance = Category.objects.create(
                    type_category=int(data['type_category']),
...
class ChangeRecurrentMonthForm(forms.ModelForm):
    ...
    def get_action(self):
        if self.instance.pk:
            return reverse(
                'edit_recurrent_month', kwargs={'pk': self.instance.pk})
        else:
            return reverse(
                'create_recurrent_month',
                kwargs={
    ...
    def _get_date_for_month(self):
        ...
        if day_of_month > last_day.day:
            ...
            day = tmz(datetime(
                day=day_of_month, month=self.month, year=self.year))
...
class InitialBalanceForm(forms.ModelForm):
    ...
    def save(self, user, commit=False):
        ...
        instance.category = Category.objects.get(
            type_category=SYSTEM_CATEGORIES, slug=self.slug_category,
            user=instance.user)
        instance.save()
        return instance
